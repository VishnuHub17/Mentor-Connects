...
function buildMappedProfile(rawProfile: Record<string, unknown>) {
  const firstName =
    typeof (rawProfile.localizedFirstName ?? rawProfile.firstName) ===
      "string"
      ? String(rawProfile.localizedFirstName ?? rawProfile.firstName)
      : null;
  const lastName =
    typeof (rawProfile.localizedLastName ?? rawProfile.lastName) === "string"
      ? String(rawProfile.localizedLastName ?? rawProfile.lastName)
      : null;
  ...
  const avatarUrl =
    profilePicture && typeof profilePicture === "object" &&
      typeof (profilePicture as Record<string, unknown>).displayImage ===
        "string"
      ? String((profilePicture as Record<string, unknown>).displayImage)
      : null;
