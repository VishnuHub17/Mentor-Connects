...
export default function AddWebinarModal({ isOpen, onClose, onAdded, mentorId }) {
  ...
  const [scheduledDate, setScheduledDate] = useState(() => sessionStorage.getItem('wm_date') || '');
  const [scheduledTime, setScheduledTime] = useState(() => sessionStorage.getItem('wm_time') || '');
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      sessionStorage.setItem('wm_date', scheduledDate);
      sessionStorage.setItem('wm_time', scheduledTime);
      sessionStorage.setItem('wm_aud', targetAudience);
      sessionStorage.setItem('wm_meet', meetLink);
    }
  }, [isOpen, title, description, scheduledDate, scheduledTime, targetAudience, meetLink]);
  ...
  const clearDraft = () => {
    ...
    sessionStorage.removeItem('wm_date');
    sessionStorage.removeItem('wm_time');
